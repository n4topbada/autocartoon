...
test("OAuth 자리표시 이메일은 관리자 비밀번호 재설정에서 제외한다", () => {
  assert.equal(canAdminResetPassword("wony@wonyframe.com"), true);
  assert.equal(canAdminResetPassword("kakao-123@oauth.wonyframe.local"), false);
});
