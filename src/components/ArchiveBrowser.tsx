...
export default function ArchiveBrowser({ active = true, embedded = false }: ArchiveBrowserProps) {
  ...
  return (
    <Root className={`${styles.page} ${embedded ? styles.embedded : ""}`}>
      <header className={`${styles.header} ${embedded ? styles.headerEmbedded : ""}`}>
        {!embedded && <Link href="/" className={styles.iconButton} title="돌아가기"><LuArrowLeft /></Link>}
        <div>
          <h1>작업 보관함</h1>
