...
| 중간 | Google/Kakao 신규 계정은 아는 비밀번호가 없는데 설정·탈퇴가 현재 비밀번호를 요구했음 | OAuth 계정의 로컬 비밀번호 설정을 제거하고, 공급자 세션으로 계정 관리·탈퇴를 허용. 비밀번호 기능은 OAuth 미연결 레거시 회원에만 유지 |
...
- Cloud Run `wonybananabot-00035-scs`: 트래픽 100%, 컨테이너 정상 기동, OAuth 전용 계정 설정 확인, 배포 후 오류 로그 0건
