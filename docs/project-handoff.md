...
- 캐릭터·제스처 설정 패널을 드래그·키보드로 조절하고 폭을 브라우저에 기억하도록 통합
...
운영 반영 상태: Cloud Run Job 실행 `wony-prisma-migrate-gwgwh`가 `20260723230000_add_password_reset_tokens`까지 총 22개 마이그레이션을 운영 DB `autocartoon`에 적용하고 exit 0으로 완료했다. 2026-07-24 리비전 `wonybananabot-00055-6l2`가 트래픽 100%를 받고 있다. 배경 전용 장면 생성, 캔버스 자동저장·페이지 배경·다중 선택·페이지 순서, 실시간 강도 미리보기가 있는 무료 코너 연결색 누끼와 Nano Banana AI 정밀 누끼를 반영했다. 로컬에서 그라데이션 배경의 무료 누끼 미리보기와 원본 해상도 적용, 실제 Nano Banana 배경 생성과 AI 누끼를 수행했고 차감·투명 PNG·좌표 보정을 확인했다. 운영 로그인 화면은 200, 비인증 `/studio`는 복귀 주소를 보존한 로그인 화면으로 이동하고 누끼 API는 401을 반환했다. 새 리비전 Cloud Run ERROR 로그는 0건이다. 로컬 자동 테스트 `142/142`, TypeScript, ESLint와 76개 페이지 프로덕션 빌드를 통과했다. 배포 소스 커밋은 `eb65222`다.
