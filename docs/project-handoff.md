...
운영 반영 상태: 기존 마이그레이션 실행 `wony-prisma-migrate-99klj`는 성공 상태다. AI 순간 제한 재시도, 배경 배치 직렬화, 콘텐츠 슬롯 집계와 불투명 흑백 마스크 보정을 담은 리비전 `wonybananabot-00032-p8p`에 트래픽 100%를 연결했다. Cloud SQL 연결, 서비스 계정, 최대 4 인스턴스, 동시성 80, 600초 제한을 그대로 유지했다. 기능 중심 운영 E2E에서 앱이 통제할 수 있는 `35/35` 항목을 통과했고 Google OAuth 비밀값과 카카오페이 사이트 도메인 두 외부 설정만 미완료다. 상세 결과는 [production-e2e-2026-07-18.md](./production-e2e-2026-07-18.md)에 있다. 이번 변경에는 DB 마이그레이션이 없다.
